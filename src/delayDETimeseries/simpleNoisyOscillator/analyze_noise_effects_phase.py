...
def analyze_timeseries_phase_noise(k, theta0, omega0, xi0, dt, tmax):
    """Analyze multiple timeseries with different phase noise levels using ensemble averaging"""
    
    script_dir = Path(__file__).parent
    
    # Find the parameter folder
    folder_pattern = f"k_{k}_theta0_{theta0}_omega0_{omega0}_xi0_{xi0}_dt_{dt}_tmax_{tmax}"
    folder_path = script_dir / "outputs/PhaseNoiseOscillator" / folder_pattern
    ...
    output_dir = script_dir / "plots/noise_analysis_phase"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Setup figure with subplots
    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    fig.suptitle(f'Phase Noise Analysis: d²θ/dt² = -ω₀²sin(θ + ξ), ω₀={k}', fontsize=14, fontweight='bold')
    
    # Colors for different noise levels
    colors = plt.cm.viridis(np.linspace(0, 1, len(eta_values)))
    
    # Store results for ensemble statistics
    damping_rates_mean = []
    damping_rates_std = []
    ...
    xi_variance_mean = []
    xi_variance_std = []
    xi_variance_eta = []  # Track eta values for xi variance
    ...
    for idx, eta in enumerate(eta_values):
        ...
        for sim_file in sim_files:
            try:
                ...
                time = data[:, 0]
                theta = data[:, 1]
                xi = data[:, 3]  # Accumulated phase noise
                ...
                all_xi_variances.append(np.var(xi_steady))
        ...
        # Plot 4: Variance of theta with error bars
        var_mean = np.mean(all_variances)
        var_std = np.std(all_variances) / np.sqrt(len(all_variances))
        
        if eta == 0.0:
            axes[1, 1].text(0.05, 0.95, f'η=0: σ²_θ={var_mean:.4f}±{var_std:.4f}', 
        ...
        # Store xi variance
        xi_var_mean = np.mean(all_xi_variances)
        xi_var_std = np.std(all_xi_variances) / np.sqrt(len(all_xi_variances))
        xi_variance_mean.append(xi_var_mean)
        xi_variance_std.append(xi_var_std)
        xi_variance_eta.append(eta)  # Track eta for xi variance
        
        # Store damping statistics
        if eta > 0 and all_damping:
            damp_mean = np.mean(all_damping)
            damp_std = np.std(all_damping) / np.sqrt(len(all_damping))
    ...
    # Format subplot 4: Variance of theta vs noise
    axes[1, 1].set_xlabel('Phase Noise Strength (η)')
    axes[1, 1].set_ylabel('Variance of θ')
    axes[1, 1].set_title('Variance of θ vs Phase Noise')
    ...
    axes[0, 2].set_xlabel('Phase Noise Strength (η)')
    ...
    # Format subplot 6: Variance of accumulated phase noise
    if len(xi_variance_mean) > 0:
        axes[1, 2].errorbar(xi_variance_eta, xi_variance_mean, yerr=xi_variance_std,
                           fmt='o', markersize=8, color='purple', capsize=5, zorder=3)
        
        # Expected quadratic relationship for Brownian motion: var(ξ) ~ η² t
        if len(variance_eta) > 1:
            # Fit power law
            log_eta = np.log10(variance_eta)
            log_var = np.log10([xi_variance_mean[i] for i, e in enumerate(xi_variance_eta) if e in variance_eta])
            coeffs = np.polyfit(log_eta, log_var, 1)
            fit_eta = np.logspace(np.log10(min(variance_eta)), np.log10(max(variance_eta)), 50)
            fit_var = 10**(coeffs[0] * np.log10(fit_eta) + coeffs[1])
            axes[1, 2].plot(fit_eta, fit_var, 'r--', linewidth=2, 
                          label=f'Power law: α={coeffs[0]:.2f}', alpha=0.7)
            axes[1, 2].legend(fontsize=9)
                
    axes[1, 2].set_xlabel('Phase Noise Strength (η)')
    axes[1, 2].set_ylabel('Variance of ξ')
    axes[1, 2].set_title('Accumulated Phase Noise Variance')
    ...
    # Save figure
    output_file = output_dir / f"noise_analysis_phase_k_{k}.png"
...
if __name__ == '__main__':
    ...
    analyze_timeseries_phase_noise(k, theta0, omega0, xi0, dt, tmax)
