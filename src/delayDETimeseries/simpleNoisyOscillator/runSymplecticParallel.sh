...
run_simulation() {
    local eta=$1
    local sim_no=$2
    ./noiseSymplectic $K $GAMMA $eta $THETA0 $OMEGA0 $DT $TMAX $sim_no > /dev/null 2>&1
}

export -f run_simulation
export K GAMMA THETA0 OMEGA0 DT TMAX
...
echo "Results saved in outputs/SimpleOscillator/"
