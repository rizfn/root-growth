...
def plot_autocorr_with_envelope(tau, k, theta0, dt, tmax):
    ...
    fig.suptitle(f'Autocorrelation Damping: Delay DE, τ={tau}, k={k}', 
    ...
    for subplot_idx, eta in enumerate(eta_values):
        ...
        # Format eta to match file naming: 0.0 -> "0", otherwise keep as is
        ...
        # Plot autocorrelation with error band
        ax.plot(lag_times, autocorr_mean, 'b-', linewidth=2, label='Autocorrelation', alpha=0.8)
        ax.fill_between(lag_times, autocorr_mean - autocorr_std, 
                       autocorr_mean + autocorr_std, alpha=0.2, color='b')
        
        # Plot envelope if we have fit parameters
        if all_params:
            params_mean = np.array(all_params).mean(axis=0)
            params_std = np.array(all_params).std(axis=0)
            ...
            ax.plot(lag_times, envelope_upper, 'r--', linewidth=2, 
                   label=f'Envelope (λ={lam:.5f})', alpha=0.8)
            ax.plot(lag_times, envelope_lower, 'r--', linewidth=2, alpha=0.8)
            
            # Add text with fit parameters
            textstr = (f'η = {eta:.4f}\n'
                      f'λ = {lam:.5f} ± {params_std[1]:.5f}\n'
                      f'ω = {omega:.4f} ± {params_std[2]:.4f}\n'
                      f'A = {A:.4f}')
        ...
        ax.set_xlabel('Lag Time', fontsize=11)
    ...
    # Save figure
    output_file = output_dir / f"autocorr_damping_tau_{tau}_k_{k}.png"
...
if __name__ == '__main__':
    ...
    plot_autocorr_with_envelope(tau, k, theta0, dt, tmax)
