...
// dθ/dt = -k * f(θ(t-τ)),  f(x) = sign(x)*|x|*exp(-|x|),  τ = 1
// Odd extension of x*exp(-x): peaks at |x|=1, decays to 0, zero only at x=0.
// Trapezoidal (2nd-order Adams) integration with circular history buffer.
// Usage: ./timeseries_xexpnegx k [theta0 t_warmup t_measure record_dt]

constexpr double TAU   = 1.0;
constexpr double DT    = 0.001;
constexpr int    N_BUF = static_cast<int>(TAU / DT) + 1;   // 1001

inline double F(double x) { double ax = std::abs(x); return std::copysign(ax * std::exp(-ax), x); }
