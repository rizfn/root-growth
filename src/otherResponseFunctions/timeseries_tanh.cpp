...
// dθ/dt = -k * f(θ(t-τ)),  f(x) = tanh(x),  τ = 1
// Bounded odd function, f(0)=0, f'(0)=1, saturates at ±1.
// Trapezoidal (2nd-order Adams) integration with circular history buffer.
// Usage: ./timeseries_tanh k [theta0 t_warmup t_measure record_dt]

constexpr double TAU   = 1.0;
constexpr double DT    = 0.001;
constexpr int    N_BUF = static_cast<int>(TAU / DT) + 1;   // 1001

inline double F(double x) { return std::tanh(x); }
