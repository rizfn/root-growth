...
def plot_wavelengths(rows, out_png, method="firstpeak"):
    """Plot wavelength distributions for tilted, vertical, and all roots."""
    out_png.parent.mkdir(parents=True, exist_ok=True)

    if method == "firstpeak":
        y_key = "wavelength_firstpeak_mm"
        method_title = "first autocorrelation peak"
        color_all = "tab:blue"
        color_tilted = "tab:orange"
        color_vertical = "tab:green"
    else:
        y_key = "wavelength_fit_mm"
        method_title = "sinusoid fit"
        color_all = "tab:purple"
        color_tilted = "tab:red"
        color_vertical = "tab:cyan"

    y_all = np.array([r[y_key] for r in rows if np.isfinite(r[y_key])], dtype=float)
    y_tilted = np.array([r[y_key] for r in rows if "AgarTilted" in r["image"] and np.isfinite(r[y_key])], dtype=float)
    y_vertical = np.array([r[y_key] for r in rows if "AgarTilted" not in r["image"] and np.isfinite(r[y_key])], dtype=float)
    ...
    # Shared bins and x-limits so all panels are directly comparable.
    ...
    datasets = [
        ("Tilted files (AgarTilted)", y_tilted, color_tilted),
        ("Vertical files (non-AgarTilted)", y_vertical, color_vertical),
        ("All roots (all files)", y_all, color_all),
    ]

    for ax, (subtitle, yy, color) in zip(axes, datasets):
        if yy.size > 0:
    ...
    fig.suptitle(f"Wavelength distributions by file group ({method_title})", y=0.995)
...
def main():
    ...
    out_png_first = output_dir / "wavelength_distribution_firstpeak.png"
    out_png_fit = output_dir / "wavelength_distribution_fit.png"
    ...
    print(f"First-peak distribution plot saved to {out_png_first}")
    print(f"Sinusoid-fit distribution plot saved to {out_png_fit}")
