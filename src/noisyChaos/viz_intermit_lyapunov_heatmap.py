...
def load_run_summaries(base_dir: str) -> pd.DataFrame:
    pattern = os.path.join(base_dir, "outputs", "intermittency_lyapunov", "run_summaries", "*.tsv")
...
def plot_lyapunov_value_heatmap(agg_df: pd.DataFrame, out_path: str):
    ...
    ax.set_title("Intermittency Lyapunov Exponent")
    ax.set_xlabel("k (intermittency parameter)")
    ax.set_ylabel("eta (noise amplitude)")
    ax.set_yscale("log")
...
def plot_lyapunov_sem_heatmap(agg_df: pd.DataFrame, out_path: str):
    ...
    ax.set_title("Intermittency Lyapunov SEM")
    ax.set_xlabel("k (intermittency parameter)")
    ax.set_ylabel("eta (noise amplitude)")
    ax.set_yscale("log")
    cb = fig.colorbar(m, ax=ax)
...
def main():
    parser = argparse.ArgumentParser(description="Aggregate and plot intermittent Lyapunov heatmap over (k, eta).")
    parser.add_argument("--base-dir", default=os.path.dirname(__file__), help="Directory containing outputs/intermittency_lyapunov")
    parser.add_argument(
        "--summary-out",
        default=os.path.join(os.path.dirname(__file__), "outputs", "intermittency_lyapunov", "cell_summary.tsv"),
        help="Output TSV for aggregated per-cell statistics",
    )
    parser.add_argument(
        "--plots-dir",
        default=os.path.join(os.path.dirname(__file__), "plots", "intermittency_lyapunov"),
