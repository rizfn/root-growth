...
def summarize_and_filter(rows, cutoff_length_mm):
	grouped = {}
	for row in rows:
	...
	for image_name in sorted(grouped):
		...
		filtered_count = filtered_nopeak + filtered_cutoff
		...
		summary_rows.append(
			{
				...
				"filtered_cutoff_length": int(filtered_cutoff),
...
def run_for_cutoff(autocorr_rows, base_output_dir, cutoff_length_mm):
	tag = value_token("cl", cutoff_length_mm)
	kept_rows, summary_rows = summarize_and_filter(autocorr_rows, cutoff_length_mm=cutoff_length_mm)
	...
	plot_filtered_fraction(kept_rows, summary_rows, frac_path, method_label=f"cutoff length <= {cutoff_length_mm:.6g} mm")
...
def main():
	...
	base_output_dir = Path("src/dataWavelengthColParam/plots/cutoff_length")
	...
	cutoff_length_mm = [5.0, 8.0, 10.0, 15.0]
	...
	values = normalize_param_values(cutoff_length_mm)
	print(f"Running cutoff-length analysis for {len(values)} value(s)")
	for val in values:
		print(f"\nCutoff length = {val:.6g} mm")
		run_for_cutoff(autocorr_rows, base_output_dir, cutoff_length_mm=val)
