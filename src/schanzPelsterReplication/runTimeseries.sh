...
T_WARMUP=10000
T_MEASURE=50000   # 500 complete 4τ windows when τ=25
...
import numpy as np
vals = np.linspace(1.4, 5.0, 100) / $TAU
print(' '.join(f'{v:.8f}' for v in vals))
")"

echo ""
echo "=== Timeseries sweep: ${#K_VALS[@]} k-values × ${#IC_VALUES[@]} ICs ==="
echo "    τ·k ∈ [1.4, 5.0],  t_warmup=$T_WARMUP,  t_measure=$T_MEASURE,  record_dt=$RECORD_DT"
