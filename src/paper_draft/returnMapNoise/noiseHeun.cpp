...
constexpr double DEFAULT_TAU = 1.0;           // Time lag
constexpr double DEFAULT_K = 2.0;              // Gravitropic strength
...
constexpr double DEFAULT_DT = 0.001;            // Time step
constexpr double DEFAULT_RECORD_DT = 0.01;      // Recording interval (write to file every record_dt)
...
int main(int argc, char *argv[])
{
    ...
    // Keep this dataset restricted to tau=1 outputs.
    if (std::abs(tau - 1.0) > 1e-12)
    {
        std::cerr << "Requested tau=" << tau << " overridden to tau=1.0" << std::endl;
        tau = 1.0;
    }
