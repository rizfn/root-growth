...
def analyze_image(filepath, dx_pixels=8, min_root_length=200, max_lag=200):
    ...
    for region in measure.regionprops(labeled):
        ...
        peak_height = float(ac[peak_lag]) if (ac is not None and peak_lag is not None and peak_lag < len(ac) and np.isfinite(ac[peak_lag])) else np.nan
...
def filter_rows_by_peak_threshold(rows, peak_threshold=0.1):
    kept = []
    for row in rows:
        wl = float(row.get("wavelength_firstpeak_mm", np.nan))
        peak_h = float(row.get("peak_height", np.nan))
        if not np.isfinite(wl):
            continue
        if (not np.isfinite(peak_h)) or (peak_h < peak_threshold):
            continue
        kept.append(row)
    return kept
