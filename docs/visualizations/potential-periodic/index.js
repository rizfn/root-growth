...
// Add Potential Strength slider
controls.append("label")
    .text(" Potential Strength: ")
    .append("input")
    .attr("type", "range")
    .attr("min", 0.1)
    .attr("max", 5)
    .attr("step", 0.01)
    .attr("value", 1)
    .attr("id", "potentialStrengthSlider")
    .style("vertical-align", "middle");
...
function computePotentialForce(tip) {
    const strength = +d3.select("#potentialStrengthSlider").property("value");
    const buffer = 10;
    const alpha = 0.3; // Controls steepness of exponential
    let fx = 0, fy = 0;
    for (let c of circles) {
        ...
        let edgeDist = dist - c.r;
        if (edgeDist < buffer) {
            // Exponential repulsion, only strong very close to the edge
            let force = strength * Math.exp(-alpha * edgeDist);
            fx += (dx / dist) * force;
            fy += (dy / dist) * force;
        }
    }
