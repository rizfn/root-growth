...
sliderContainer.html(`
    ...
    <label style="font-size:2em;">
        ...
        <input type="range" id="divisionZoneSlider" min="1" max="20" value="5" step="1" style="vertical-align:middle; margin-left:8px;">
        <span id="divisionZoneValue">5</span>
    </label>
`);
...
let radius = 15;
let divisionZoneLength = 5;

// --- 3. Parameter grid for simulations ---
const elongationZoneLengths = d3.range(0, 12, 1);
...
function simulateRoot(elongationZoneLength, downwardReturn, spacing, radius, divisionZoneLength, rootSpeed) {
    const rowHeight = Math.sqrt(3) * spacing / 2;
    let circles = [];
    let row = 0;
    for (let y = radius; y < latticeHeight + rowHeight; y += rowHeight, row++) {
...
d3.select("#divisionZoneSlider").on("input", function () {
    divisionZoneLength = +this.value;
    d3.select("#divisionZoneValue").text(divisionZoneLength);
    runAllSimulations();
    drawHeatmap();
    if (selectedResult) {
        const updated = allResults.find(
