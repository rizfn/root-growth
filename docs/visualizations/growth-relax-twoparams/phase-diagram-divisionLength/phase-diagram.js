...
sliderContainer.html(`
    ...
    <label style="font-size:2em;">
        <span>Elongation Zone:</span>
        <input type="range" id="elongationZoneSlider" min="0" max="20" value="5" step="1" style="vertical-align:middle; margin-left:8px;">
        <span id="elongationZoneValue">5</span>
    </label>
`);
...
let radius = 15;
let elongationZoneLength = 5;

// --- 3. Parameter grid for simulations ---
const divisionZoneLengths = d3.range(1, 24, 1);
const downwardReturns = d3.range(0.1, 0.51, 0.025);
...
const rootSpeed = 2;
let allResults = []; // {divisionZoneLength, downwardReturn, rootLine, slope}

function simulateRoot(divisionZoneLength, downwardReturn, spacing, radius, elongationZoneLength, rootSpeed) {
    const rowHeight = Math.sqrt(3) * spacing / 2;
    let circles = [];
    let row = 0;
    for (let y = radius; y < latticeHeight + rowHeight; y += rowHeight, row++) {
    ...
    return { divisionZoneLength, downwardReturn, rootLine, slope };
}
...
function runAllSimulations() {
    ...
    allResults = [];
    for (let i = 0; i < divisionZoneLengths.length; ++i) {
        for (let j = 0; j < downwardReturns.length; ++j) {
            const divisionZoneLength = divisionZoneLengths[i];
...
function drawHeatmap() {
    ...
    svgHeatmap.selectAll("rect")
        .data(allResults)
        .join("rect")
        .attr("x", d => xScale(d.divisionZoneLength))
...
d3.select("#spacingSlider").on("input", function () {
    ...
    if (selectedResult) {
        ...
        const updated = allResults.find(
            d => d.divisionZoneLength === selectedResult.divisionZoneLength &&
...
d3.select("#elongationZoneSlider").on("input", function () {
    elongationZoneLength = +this.value;
    d3.select("#elongationZoneValue").text(elongationZoneLength);
    runAllSimulations();
    drawHeatmap();
    if (selectedResult) {
        const updated = allResults.find(
